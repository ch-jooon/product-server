...
**도메인을 중심**으로 설계되어 비즈니스 로직과 인프라 로직을 명확하게 분리합니다.

테스트하기 쉬운 구조를 고민했고, 각 모듈은 독립적으로 테스트할 수 있습니다.

또한 조회 성능을 위해 **캐시**를 적용하고, **확장성**을 고려하여 설계되었습니다.

다만, 관리자 기능은 단순 CRUD 기능이 주를 이루기 때문에 `클린 아키텍처`를 따르지 않고, 최대한 단순하게 `레이어드 아키텍처`로 구현했습니다.

관리자 기능은 `product-api` 모듈 내 `product.api.admin` 패키지에 모두 위치하고 있습니다.
...
도메인 중심의 비즈니스 객체를 통해 필요한 의존성만 주입받아 개별 비즈니스 로직 테스트를 쉽게 할 수 있도록하고, 새로운 비즈니스 요구사항을 쉽게 추가할 수 있는 구조를 지향합니다.  
